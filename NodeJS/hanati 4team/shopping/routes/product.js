    ...
    if(database.db){
        database.ProductModel.findAll(function(err,results){
            ...
            if(results){
				...
				var userInfo;
				if(req.session.user) {
					userInfo = {id:req.session.user.id,
								   mcode:req.session.user.mcode};
				} else {
					userInfo = {}; 
				}
...
var showProductInfo = function(req,res){
    ...
    if(database.db){
        database.ProductModel.findByPcode(paramCode,function(err,result){
            if(err){
                console.log('showProduct - findByPcode에서 오류');
                return;  
            }
            ...
            if(result){
				...
				if(req.session.user) {
					userInfo.id = req.session.user.id;
					userInfo.mcode = req.session.user.mcode;
